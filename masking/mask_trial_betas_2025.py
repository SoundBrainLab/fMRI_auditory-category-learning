...
parser.add_argument("--window", 
                    help=("which model to operate on "
                          "(options: run, grouped-runs, session)"), 
                    type=str)
...
for rx, roi in enumerate(tian_sc_S2_roi_list):
    roi_dict_tian_S2[roi] = rx+1
...
for mx, mask_descrip in enumerate(roi_list):
    ...
    # run-all stat maps
    if window_descrip=='session':
        stat_maps = sorted(glob(nilearn_sub_dir+f'/*di*map-{stat_descrip}.nii.gz')) 
        ...
        for sx, stat_fpath in enumerate(stat_maps):
            ...
            # save out
            out_fbase = f'sub-{subject_id}_space-{space_label}_mask-{mask_descrip}_run-all_cond-{cond_label}_map-{stat_descrip}'
    ...
    elif window_descrip=='run':
        for rx, run_dir in enumerate(sorted(glob(nilearn_sub_dir+'/run*'))):
            stat_maps = sorted(glob(run_dir+f'/sub*/*Di*stat-{stat_descrip}_statmap.nii.gz')) 
            print('# of stat maps: ', len(stat_maps))    

            conditions_all = [(os.path.basename(x).split('_')[4]) for x in (stat_maps)]
            print('conditions_all:', conditions_all)
            ...
            for sx, stat_fpath in enumerate(stat_maps):
                ...
                # save out
                out_fbase = f'sub-{subject_id}_space-{space_label}_mask-{mask_descrip}_run-{rx:02}_cond-{cond_label}_map-{stat_descrip}'
    ...
    # run-specific stimulus stat maps
    elif window_descrip=='grouped-runs':
        if 'per_run' in model_descrip:
            for rx, rungroup_label in enumerate(['early', 'middle', 'final']):
                print('run group:', rungroup_label)

                run_dir = os.path.join(nilearn_sub_dir, 'run-grouped')
                stat_maps = sorted(glob(run_dir+f'/*rungroup-{rungroup_label}*Di*stat-{stat_descrip}_statmap.nii.gz')) 
                print('# of stat maps: ', len(stat_maps))    
                ...
                for sx, stat_fpath in enumerate(stat_maps):
                    ...
                    # save out
                    out_fbase = (f'sub-{subject_id}_space-{space_label}_mask-{mask_descrip}'+
                                 f'_rungroup-{rungroup_label}_cond-{cond_label}_map-{stat_descrip}')
                    ...
                    masked_img = masker.inverse_transform(fmri_masked)
                    nib.save(masked_img, masked_out_fpath)
        elif 'grouped-runs' in model_descrip:
            for rx, run_dir in enumerate(sorted(glob(nilearn_sub_dir+'/*'))):
                rungroup_label = os.path.basename(run_dir)
                print('run group:', rungroup_label)

                stat_maps = sorted(glob(run_dir+f'/*Di*stat-{stat_descrip}_statmap.nii.gz')) 
                print('# of stat maps: ', len(stat_maps))    
