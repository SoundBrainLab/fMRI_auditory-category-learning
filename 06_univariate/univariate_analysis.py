...
def _filter_low_quality_runs(imgs, events, confounds, sample_masks, min_retained_frac=0.5):
    ''' drop runs that retain too little data after scrubbing before fitting '''
    keep_idx = []
    for rx, (conf, mask) in enumerate(zip(confounds, sample_masks)):
        n_vols = len(conf)
        n_retained = n_vols if mask is None else len(mask)
        if (n_retained / n_vols) >= min_retained_frac:
            keep_idx.append(rx)
        else:
            print('excluding run %d: below %.0f%% volumes-retained threshold after scrubbing' % (
                rx, 100 * min_retained_frac))
    return ([imgs[i] for i in keep_idx], [events[i] for i in keep_idx],
            [confounds[i] for i in keep_idx], [sample_masks[i] for i in keep_idx])
...
def nilearn_glm_across_runs(stim_list, task_label, models, models_run_imgs, \
    ...
    for midx in range(len(models)):
        for sx, stim in enumerate(stim_list):
            ...
            imgs, events, confounds, sample_masks = _filter_low_quality_runs(
                imgs, events, confounds, sample_masks)
